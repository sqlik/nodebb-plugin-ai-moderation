...
const DEFAULTS = {
	...
	customRules: '',
	categoryActions: '{"spam":"flag","toxicity":"flag","nsfw":"flag","pii":"flag","promotion":"flag"}',
	systemReporterUid: 1,
	cidOverrides: '{}',
...
async function seedMissingDefaults(stored) {
	...
	if (seeded > 0) {
		try {
			await meta.settings.set('ai-moderation', patch);
			winston.verbose('[plugin/ai-moderation] seeded ' + seeded + ' default setting(s) to DB');
		} catch (err) {
			winston.warn('[plugin/ai-moderation] failed to seed defaults: ' + err.message);
		}
	}
	return stored;
}
...
exports.load = async () => {
	let stored = await meta.settings.get('ai-moderation') || {};
